...
If ($Package) {
    ...
    Write-NewEventlog -eventid 7005 -EntryType 'Error' -message "Install-DnsFilter.ps1 Stopped because DNSFilter is already Installed"
...
If ($Package) {
    Write-Log -message 'DNSFilter is installed'
    Write-NewEventlog -eventid 7010 -message "Install-DnsFilter.ps1 Completed Successfully"
    Clear-Files
    Return 'Installed'
} else {
    Write-Log -message 'DNSFilter Could not be installed' -type ERROR
    Write-NewEventlog -eventid 7005 -EntryType 'Error' -message "Install-DnsFilter.ps1 Could not install DNSFilter"
    Clear-Files
    Return 'Failed'
}
