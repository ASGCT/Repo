...
If (!($bootstraploaded)){
    ...
    $BaseRepoUrl = (Invoke-webrequest -URI -UseBasicParsing "https://raw.githubusercontent.com/ASGCT/Repo/main/Environment/Bootstrap.ps1").Content
...
$UID = Get-ChildItem -Path HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall, HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall | Get-ItemProperty | Where-Object {$_.DisplayName -match '*WebTitan*' } | Select-Object -ExpandProperty UninstallString
If (!($UID)) {
